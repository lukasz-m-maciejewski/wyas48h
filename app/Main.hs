...
parseOctNumber =
  Number . fromJust . numberConverter (NumberBase 8) <$> many1 octDigit


parseHexNumber :: Parser LispVal
parseHexNumber =
  Number . fromJust . numberConverter (NumberBase 16) <$> many1 hexDigit
...
numberConverter base = foldM (foldingFunction (unBase base)) 0
 where
  ...
  digitValue b d = elemIndex (toLower d) "0123456789abcdef" >>= cutoffToBase b
...
parseExpr =
  parseAtom <|> parseString <|> parseNumber <|> parseQuoted <|> parseParensList
...
eqv [List arg1, List arg2] =
  return $ Bool $ (length arg1 == length arg2) && and
    (zipWith eqvPair arg1 arg2)
 where
...
applyProc _                 = error "umphf"

makePort :: IOMode -> [LispVal] -> IOThrowsError LispVal
makePort mode [String filename] = fmap Port $ liftIO $ openFile filename mode
makePort _    _                 = error "umphf"
...
readProc _           = error "umphf"

writeProc :: [LispVal] -> IOThrowsError LispVal
writeProc [obj]            = writeProc [obj, Port stdout]
writeProc [obj, Port port] = liftIO $ hPrint port obj >> return (Bool True)
writeProc _                = error "umphf"

readContents :: [LispVal] -> IOThrowsError LispVal
readContents [String filename] = liftM String $ liftIO $ readFile filename
readContents _                 = error "umphf"
...
primitiveBindings = nullEnv >>= flip bindVars (ioFunctions ++ basicFunctions)
 where
  makeFunc' constructor (var, func) = (var, constructor func)
  ioFunctions    = map (makeFunc' IOFunc) ioPrimitives
  basicFunctions = map (makeFunc' PrimitiveFunc) primitives
